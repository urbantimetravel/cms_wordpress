...
class Profile_Magic_Basic_Functions 
{
    ...
    public function pg_get_extension_button($plugin)
    {
        ...
        if($data->is_pg_extension_activate)
        {
            if($plugin !='FRONTEND_GROUP_MANAGER' && $plugin !='MULTI_ADMINS' && $plugin !='EVENTS' && $plugin !='MENU_RESTRICTIONS' && $plugin !='HERO_BANNER' && $plugin !='RECENT_SIGNUP'):
        ?>
            <li>
        ...
        {
            if($plugin=='CUSTOM_PROFILE_SLUG' || $plugin=='DISPLAY_NAME' || $plugin=='BBPRESS' || $plugin=='WOOCOMMERCE' || $plugin=='EVENTS' || $plugin=='LOGIN_LOGOUT_MENU' || $plugin=='DEMO_CONTENT' || $plugin=='HERO_BANNER' || $plugin=='USER_ACTIVITIES' || $plugin=='RECENT_SIGNUP')
            {
